...
#
# ./Exercise2.py <n_tests> <n_points> 0 filename.csv
...
def linear_regression_training (x1, y1, x2, y2, training_size):
    weights, coeffs, prob = linreg.linear_regression(x1, y1, x2, y2, training_size) 
    return (weights, coeffs, prob) 
...
for t in range(1,tests+1):
    ...
    weights, coeffs, disagreement = linear_regression_training(x1, y1, x2, y2, points)
    line = str(t) + ',' + str(disagreement) + '\n'
    tmp_file.write (line)
    dis[t-1] = disagreement

out_disagreement = linreg.linear_regression_test(weights, coeffs, 1000)

its_pla = np.empty(1000)
dis_pla = np.empty(1000)
for t in range(1,1001):
    iter_pla, prob_pla = pla.perceptron2(coeffs, weights, 1000)
    its_pla[t-1] = iter_pla
    dis_pla[t-1] = prob_pla
...
# print output
print "disagreement (mean +- std): %f +- %f" % (np.mean(dis), np.std(dis))
print "out of sample disagreement: %f" % out_disagreement
print "average pla convergence: %f" % np.mean(its_pla)
